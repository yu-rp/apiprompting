...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output = torch.einsum("bnqk,bnkd->bnqkd", attn_weights, value_states)

        if hasattr(self, "hook_manager"):
            attn_output = self.hook_manager("matmul_v", ret = attn_output)

        if attn_output.size() != (bsz, self.num_heads, q_len, kv_seq_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads, q_len, kv_seq_len, self.head_dim)}, but is"
                f" {attn_output.size()}"
            )

        attn_output = attn_output.permute(0,2,3,1,4).contiguous()
        attn_output = attn_output.reshape(bsz, q_len, kv_seq_len, self.hidden_size)
